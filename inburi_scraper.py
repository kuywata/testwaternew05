# inburi_scraper.py (โค้ดฉบับแก้ไขสมบูรณ์)
...
DATA_FILE = "last_inburi_data.txt" # ใช้ชื่อไฟล์เดิมที่ workflow คุ้นเคย
...
def get_water_level_data():
    ...
    try:
        print(f"กำลังดึงข้อมูลสถานี {STATION_CODE} ({STATION_NAME}) จาก API ที่เสถียร...")
        ...
        # ตรวจสอบว่าค่าที่จำเป็นไม่เป็น None
...
if __name__ == "__main__":
    print("===== เริ่มการทำงานของระบบแจ้งเตือนระดับน้ำอินทร์บุรี =====")
    ...
    if current_data:
        if not last_data or last_data.get("time") != current_data.get("time"):
            print("พบข้อมูลอัปเดตใหม่! กำลังเตรียมส่งแจ้งเตือน...")
