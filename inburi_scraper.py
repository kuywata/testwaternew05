...
def get_inburi_river_data():
    ...
    try:
        ...
        # 1. สร้าง Header ที่สมบูรณ์เพื่อเลียนแบบเบราว์เซอร์
        base_headers = {
        ...
        # 2. เข้าหน้าเว็บหลักเพื่อรับ Session และ Cookies
        ...
        # 3. อ่านค่า XSRF-TOKEN จากคุกกี้ที่เซิร์ฟเวอร์ส่งมาให้
        xsrf_cookie = session.cookies.get('XSRF-TOKEN')
        if not xsrf_cookie:
            print("Fatal Error: Could not find 'XSRF-TOKEN' cookie. The website's security may have changed.")
            return None
        
        csrf_token = urllib.parse.unquote(xsrf_cookie)
        print("Successfully retrieved XSRF token from cookie.")

        # 4. สร้าง Header สำหรับยิง API
